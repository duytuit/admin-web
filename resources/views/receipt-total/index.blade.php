...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Sổ quỹ tiền mặt</li>
    </ol>
</section>
            ...
            <form id="form-search-advance" action="{{route('admin.receipttotal.index')}}" method="GET">
                <div id="search-advance" class="search-advance">
                    <div class="row form-group pull-right">
                        <div class=" col-md-12 ">
                            <a href="{{route('admin.receipt.exportFilterSoQuyTienMat',Request::all())}}" class="btn btn-success">Xuất ra excel</a>
                ...
                <table class="table table-hover table-striped table-bordered">
                    ...
                    <tbody>
                        @foreach($receiptTotals as $lock => $receiptTotal)
                            <tr>
                                <td>{{ ($lock + 1) }}</td>
                                <th>{{ @$receiptTotal->apartment->name }}</th>
                                ...
                                <td class="text-right">{{ @$receiptTotal->type == "phieu_chi" || @$receiptTotal->type == "phieu_chi_khac" || @$receiptTotal->type == "phieu_hoan_ky_quy" ? number_format(@$receiptTotal->cost) : 0 }}</td>
                                <td class="text-right">{{ number_format(@$receiptTotal->ton, 0, ",", ",") }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
