            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        @endif
                        {!! Form::open(['url' =>[route('admin.system.menu.update', $item->id) ] , 'method'=> 'POST','files' => true]) !!}
                        <div class="form-group">
                            {!! Form::label('name', 'Menu name') !!}
                            {!! Form::text('name', old('name') ?? @$item->name, ['class' => 'form-control', 'placeholder' => 'Menu name']) !!}
                        </div>
                        {{-- name --}}

                        <div class="form-group">
                            {!! Form::label('description', 'Description') !!}
                            {!! Form::text('description', old('description')?? @$item->description, ['class' => 'form-control', 'placeholder' => 'Description']) !!}
                        </div>
                            <div class="form-group">
                        ...
                        {{-- description --}}
                            @if($item->type>0)
                                <div class="form-group">
                                    ...
                                    <div class="radio">
                                        <label for="type_no"> <input name="type" type="radio" value="0" id="type_no" {{ $item->type == 0 ? 'checked' : ''}}> No</label>
                                    </div>
                                    <div class="radio">
                                        <label for="type_yes"><input name="type" type="radio" value="1" id="type_yes" {{ $item->type == 1 ? 'checked' : ''}}> Yes</label>
                                    </div>
                                </div>
                            @endif
