    ...
    <div class="box box-primary">
        <div class="box-body font-weight-bold">
            ...
            <div class="pull-right">
                ...
                @if( in_array('admin.receipt.createPhieuChi',@$user_access_router))
                <a href="{{ route('admin.receipt.createPhieuChi') }}" class="btn btn-warning">Lập phiếu chi</a>
                @endif
        ...
        <div class="box-body">
            ...
            <form id="form-search-advance" action="{{route('admin.receipt.index')}}" method="get">
                            ...
                            <tr @if($receipt->deleted_at != null) class="danger" style="text-decoration: line-through;" @endif>
                                ...
                                <td>
                                    @if($receipt->deleted_at == null)
                                        <a style="float: left" href="{{route('admin.receipt.edit',$receipt->id)}}" class="btn btn-sm btn-warning margin-r-5">
                                            <i class="fa fa-edit"></i>
                                        </a>

                                    @endif
                                      @if( in_array('admin.receipt.destroy',@$user_access_router) && $receipt->deleted_at == null)
                                        <form action="{{route('admin.receipt.destroy',$receipt->id)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                ...
                                <td>
                                    ...
                                    ?>
                                    @if(isset($receipt->data) || $receipt->vnp_transactionno == 'phieu_thu_chi')
                                        <a style="float: left" href="{{ route('admin.receipt.receiptCode',$receipt->receipt_code) }}" target="_blank"
                                            class="btn btn-sm btn-info margin-r-5"><i class="fa fa-print"></i></a>
                                    @else
                                        <a style="float: left"  href="{{isset($receipt->url) && $receipt->url && str_contains($receipt->url,'phieuthu') ? asset("/$receipt->url")  : $receipt->url}}" target="_blank"
                                            class="btn btn-sm btn-info margin-r-5"><i class="fa fa-print"></i></a>
                                    @endif
                                </td>
                            </tr>
