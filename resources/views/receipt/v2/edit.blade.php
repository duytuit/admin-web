    ...
    <section class="content">
        <div class="box box-primary">
            <section class="invoice border-bill">
                <form action="{{ route('admin.v2.receipt.update', $receipt->id) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="row">
                        <div class="col-xs-12">
                        ...
                        <div class="col-xs-12">
                            ...
                            @endif
                            <h5 class="text-center"><i>Ngày {{ date('d') }} tháng {{ date('m') }} năm
                                    {{ date('Y') }}</i></h5>
                        </div>
                        <!-- /.col -->
                    </div>
                                        ...
                                        <div class="form-group {{ $errors->has('type_payment') ? ' has-error' : '' }}">
                                            <select class="form-control" name="type_payment">
                                                @foreach ($typeReceipt as $key => $value)
                                                    <option value="{{ $value->config }}"
                                                        @if (@$receipt->type_payment == $value->config) selected @endif>
                                                        {{ $value->title }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <tr>
                                    ...
                                    <td width="80%">
                                        @php
                                            ...
                                            $total_receipt = @$receipt->PaymentDetail->sum('paid') + $coin;
                                        @endphp
                                        <div class="form-group {{ $errors->has('cost') ? ' has-error' : '' }}">
                                            <input type="text" class="form-control"
                                                value="{{ $receipt->type == 'phieu_thu_truoc' || $receipt->type == 'phieu_chi_khac' || $receipt->type == 'phieu_thu_ky_quy' || $receipt->type == 'phieu_hoan_ky_quy' ? number_format($receipt->cost) : number_format($total_receipt) }}"
                                                disabled="disabled">
                                            @if ($errors->has('cost'))
                                                <span class="help-block">
                                ...
                                <tr>
                                    ...
                                    <td width="80%">
                                        <div class="form-group {{ $errors->has('create_date') ? ' has-error' : '' }}">
                                            <input type="text" name="create_date" class="form-control date_picker"
                                                placeholder="Ngày hạch toán..." value="{{ $receipt->create_date }}">
                    ...
                    <div class="box-footer">
                        <a href="{{ route('admin.v2.receipt.index') }}" type="button"
                            class="btn btn-default pull-left">Quay
                            lại</a>
