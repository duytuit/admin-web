                ...
                <div class="col-md-12">
                    ...
                    <div class="box box-primary" style="box-shadow: none;">
                        <div class="box-header with-border text-center bg-primary">
                            <h4 class="text-create-recipt">Phiếu điều chỉnh</h4>
                        </div>
                        <!-- /.box-header -->
                                        ...
                                        <form id="form-receipt_apartment"
                                            ...
                                            <table class="table no-margin" id="table_receipt_debit">
                                                ...
                                                <tbody class="list_info">
                                                    ...
                                                    <tr>
                                                        <td colspan="8" style="font-style: italic;">Dịch vụ đã hạch toán:
                                                        </td>
                                                    </tr>
                                                    @if ($receipt_apartments)
                                                        @foreach ($receipt_apartments as $value)
                                                            <tr class="checkbox_parent">
                                                                @php
                                                                    ...
                                                                    $service = $service_apartment ? $serviceRepository->getInfoServiceById($service_apartment->bdc_service_id) : null;
                                                                    $vechicle = $service_apartment && $service_apartment->bdc_vehicle_id > 0 ? $vehicle->find($service_apartment->bdc_vehicle_id) : null;
                                                                    $datetime = date('d/m/y', strtotime($value->from_date)) . ' - ' . date('d/m/y', strtotime($value->to_date));
                                                                @endphp
                                                                <td></td>
                                                                <td>
                                                                    <input type="checkbox" name="bdc_apartment_service_price_id" value="tien_dich_vu_{{$value->id}}"
                                                                        class="checkSingle check_box_debit_receipt"
                                                                        onclick="check_chose(this)" />
                                                                </td>
                                                                <td>{{ $service ? @$service->name : 'Chưa chỉ định' }}
                                                                ...
                                                                <td> <input type="text" class="form-control paid_payment" value="{{number_format($value->paid)}}" /></td>
                                                            </tr>
                                                        @endforeach
                                                    @else
                                                        <tr style="text-align: center;">
                                            ...
                                            <div class="row mbm">
                                                @if ($receipt_apartments)
                                                    <div class="col-sm-3">
                                                        <span class="record-total">Hiển thị {{ $receipt_apartments->count() }} /
                                                            {{ $receipt_apartments->total() }} kết quả</span>
                                                    </div>
                                                    <div class="col-sm-6 text-center">
                                                        <div class="pagination-panel">
                                                            {{ $receipt_apartments->appends(request()->input())->links() }}
                                                        </div>
                                                    </div>
                                                    <div class="col-sm-3 text-right">
                                                        <span class="form-inline">
                                                            ...
                                                            <select name="per_page" class="form-control"
                                                                data-target="#form-receipt_apartment">
                                                                @php $list = [10, 20, 50, 100, 200]; @endphp
                                                                @foreach ($list as $num)
                                                                    <option value="{{ $num }}"
                                                                        {{ $num == $per_page ? 'selected' : '' }}>
                                                                        {{ $num }}</option>
                                                                @endforeach
                                                            </select>
                                                        </span>
...
@section('javascript')
    ...
    <script>
        ...
        $('.tao_phieu_dieu_chinh').click(function(e){
            ...
            showLoading();
            $.ajax({
                url: '/admin/v2/receipt/save_adjustment_slip',
                type: 'POST',
                data: {
