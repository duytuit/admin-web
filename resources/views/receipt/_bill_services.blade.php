...
<table id="table_receipt_debit" class="table no-margin">
    <thead>
        <tr>
            <th>
                <input type="checkbox" class="checkServiceAll_v1" onclick="checkServiceAll_v1(this)" data-target=".checkSingle" />
            </th>
            <th>Dịch vụ</th>
                ...
                <tr class="checkbox_parent">
                    <td>
                        @if($billDetail->new_sumery != 0 && $billDetail->is_free == 0)
                            <input type="checkbox" name="ids[]" id="check_box_debit_receipt" onclick="checkService(this)" />
                        @endif
                    </td>
                    <td>{{ $service->name }}</td>
                    <td>{{ $datetime }}</td>
