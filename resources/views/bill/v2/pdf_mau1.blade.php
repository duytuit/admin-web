...
<div class="container">
    ...
    <div class="footer">
        <table style="width: 100%; padding: 10px" border="1">
            <tbody>
            <tr>
                <td class="padding-tb"><b>Tên dịch vụ</b></td>
                <td class="padding-tb"><b>Đơn giá (*)</b></td>
                <td class="padding-tb"><b>Đơn giá / ngày</b></td>
                <td class="padding-tb"><b>Số lượng</b></td>
                <td class="padding-tb"><b>Thành tiền</b></td>
            </tr>
            @php
                $totalService = 0;
                $totalVehicle = 0;
            ...
            @if(count($debit_detail['service']) > 0)
                ...
                <tr>
                    ...
                    <td class="padding-tb" style="text-align: right;"><strong>{{ number_format($totalService) }}</strong></td>
                </tr>
                @foreach($debit_detail['service'] as $key => $service)
                    @php
                        // $totalDv = $service->is_free == 0 ? $service->sumery + $service->previous_owed : 0;
                    ...
                    <tr>
                        ...
                        <td class="padding-tb">
                            ...
                            @if($service->apartmentServicePrice->floor_price > 0)
                                ({{ number_format($service->apartmentServicePrice->floor_price) }} * {{ $apartment->area }}m2)
                            @endif
                        </td>
                        <td class="padding-tb" style="text-align: right;">{{ number_format($service->price) }}</td>
                        <td class="padding-tb">{{ $service->quantity }}</td>
                        <td class="padding-tb"  style="text-align: right;">
            ...
            @if(count($debit_detail['vehicle']) > 0)
                ...
                <tr>
                    ...
                    <td class="padding-tb" style="text-align: right;"><strong>{{ number_format($totalVehicle) }}</strong></td>
                </tr>
                @foreach($debit_detail['vehicle'] as $key => $service)
                    @php
                        // $totalVi = $service->is_free == 0 ? $service->sumery + $service->previous_owed : 0;
            ...
            @if(count($debit_detail['other']) > 0)
                @foreach($debit_detail['other'] as $diennuoc)
                    ...
                    <tr>
                        ...
                        <td class="padding-tb" style="text-align: right;"><strong>{{ number_format(@$diennuoc->sumery) }}</strong></td>
                    </tr>
                    @foreach (@$detail->data as $key => $item)
                        <tr>
                            <td class="padding-tb">- Từ {{ @$item->from }} - {{ $item->to }}</td>
                            <td class="padding-tb">{{ number_format(@$item->price) }}</td>
                            <td class="padding-tb">-</td>
                            <td class="padding-tb">{{ @$item->to - @$item->from + 1 }}</td>
                            <td class="padding-tb" style="text-align: right;">{{ number_format(@$item->total_price) }}</td>
                        </tr>
                    @endforeach
                @endforeach
            @endif
             @if(count($debit_detail['first_price']) > 0)
                ...
                @foreach($debit_detail['first_price'] as $key => $service)
                    ...
                    <tr>
                        ...
                        <td class="padding-tb" >{{ number_format($service->price) }}</td>
                        <td class="padding-tb">{{ $service->quantity }}</td>
                        <td class="padding-tb"  style="text-align: right;">
                            {{ number_format($service->sumery) }}
                            @if($service->is_free == 1)
            ...
            @endif
            <tr>
                <td colspan="4" class="padding-tb"><b>(A)TỔNG CỘNG</b></td>
                @php
                    $total = $totalService + $totalVehicle + $totalPrice;
                @endphp
                <td class="padding-tb" style="text-align: right;">{{ number_format($total) }}</td>
            </tr>
            <tr>
                <td colspan="4" class="padding-tb"><b>(B)ĐÃ THANH TOÁN</b></td>
                <td class="padding-tb" style="text-align: right;">
                    @php
                        $paid = 0;    
                        foreach (@$debitDetails as $_debitDetail){
                            $paid = $paid + $_debitDetail->total_paid;
                        }
                    @endphp
                    {{ number_format($paid) }}
                </td>
            ...
            <tr>
                <td colspan="4" class="padding-tb"><b>(C)NỢ CŨ</b></td>
                @php
                    $no = @$totalPaymentDebit[0]->total_payment > 0 ? $totalPaymentDebit[0]->total_payment : 0;
                @endphp
                <td class="padding-tb"  style="text-align: right;">{{ number_format($no) }}</td>
            </tr>
            <tr>
                <td colspan="4" class="padding-tb"><b>(D)DƯ CŨ</b></td>
                @php
                    $du = @$totalPaymentDebit[0]->total_payment < 0 ? $totalPaymentDebit[0]->total_payment : 0;
                @endphp
                <td class="padding-tb"  style="text-align: right;">{{ number_format(abs($du)) }}</td>
            </tr>
            <tr>
                <td colspan="4" class="padding-tb"><b>(E)CẦN THANH TOÁN (A-B+C-D)</b></td>
                <td class="padding-tb" style="text-align: right;">{{ number_format($total - $paid + $no - abs($du)) }}</td>
            </tr>
            </tbody>
        </table>
