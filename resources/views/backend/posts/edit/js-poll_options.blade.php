<script>
    ...
    function get_data_select_user(options) {
        $(options.object).select2({
            ajax: {
                url: '@if( in_array('admin.polloptions.getAll',@$user_access_router)){{ route("admin.polloptions.getAll") }}@endif',
                dataType: 'json',
    ...
    $('.btn-save-option').click(function(e){
        ...
        $.ajax({
            url: "@if( in_array('admin.posts.save.option',@$user_access_router)){{ route("admin.posts.save.option") }}@endif",
            type: 'POST',
            data: {
                _token: _token,
                post_id: post_id,
