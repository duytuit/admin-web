                           ...
                           @if($billDetail->count() > 0)
                                ...
                                <tr>
                                    ...
                                    <td>
                                       ...
                                       @endif
                                        @if( in_array('admin.debit.detailDebit.delete.version',@$user_access_router) && collect($billDetail)->where('bdc_service_id',$debit->bdc_service_id)->max('version') == $debit->version)
                                                <a href="{{ route('admin.debit.detailDebit.delete.version',['id'=>$debit->id]) }}" class="btn btn-sm btn-danger" title="xóa" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')"><i class="fa fa-times"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
