...
@section('content')
    <section class="content-header">
        <h1>
            Quản lý
            <small>Chọn dịch vụ công ty</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="box box-primary">
            <div class="box-body">
                <form action="{{route('admin.service.company.postChoose')}}" method="post">
                    @csrf
                        ...
                        <table class="table table-hover table-striped table-bordered">
                            ...
                            @if($services->count() > 0)
                                @foreach($services as $key => $service)
                                        <tr>
                                            <td><input @if(in_array($service->service_code, array_keys($companyServices))) checked @endif @if(in_array($service->service_code, array_keys($companyServices)) && $companyServices[$service->service_code]) disabled @endif type="checkbox" name="ids[]" value="{{ $service->id }}"
                    ...
                    <div class="row form-group">
                        <div class="col-sm-4">
                            <a style="border: none" href="{{ route('admin.service.company.create') }}"><i
                                        class="fa fa-plus" aria-hidden="true"></i>
                                Thêm dịch vụ công ty</a>
                        </div>
                        ...
                        <div class="col-sm-2 text-center">
                            <a href="{{ route('admin.service.company.index') }}" class="btn btn-warning"><i
