            ...
            <div class="box-body">
                <div class="row form-group">
                    <div class="col-sm-4 col-xs-12">
                        <form action="{{route('admin.service.building.index')}}" method="get">
                    ...
                    <div class="col-sm-8">
                        <a href="{{route('admin.service.apartment.index')}}" class="btn btn-primary margin-r-5">Căn hộ
                            sử dụng dịch vụ</a>
                        <a href="{{ route('admin.service.building.importexcel') }}"
                        class="btn btn-info pull-right margin-r-5"><i class="fa fa-edit"></i>
                            Import dịch vụ cho căn hộ</a>
                        <a href="{{route('admin.service.building.choose')}}"
                           class="btn btn-success pull-right margin-r-5">Chọn dịch
                            vụ</a>
                        <a href="{{ route('admin.service.building.create') }}"
                           class="btn btn-info pull-right margin-r-5"><i class="fa fa-edit"></i>
                            Thêm mới</a>
                    </div>
                </div>
                <div class="row form-group">
                    ...
                    <div class="col-sm-6">
                        <a class="btn btn-warning change-status"
                           data-action="{{route('admin.service.building.status')}}" data-method="Active"><i
                                    class="fa fa-check"></i> Active</a>
                        <a class="btn btn-success change-status"
                           data-action="{{route('admin.service.building.status')}}" data-method="Inactive"><i
                                    class="fa fa-times"></i> Inactive</a>
                        <a class="btn btn-primary order_index_accounting"><i class="fa fa-bars"></i> Sắp xếp ưu tiên hạch toán tự động</a>
                    </div>
                </div>
                                ...
                                @foreach($services as $key => $service)
                                    <tr class="row1" data-id="{{ $service->id }}">
                                        ...
                                        <td>
                                            <div class="onoffswitch">
                                                <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" data-id="{{ $service->id }}"
                                                       id="myonoffswitch_{{ $service->id }}" data-url="{{ route('admin.service.building.status') }}" value="{{$service->status}}" @if($service->status == true) checked @endif >
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('admin.service.building.edit', ['id' => $service->id]) }}"
                                               class="btn btn-sm btn-info"><i class="fa fa-edit"></i></a>
                                            {{-- @if(\Auth::user()->isadmin == 1)
                                            <a href="{{ route('admin.service.building.destroy', ['id' => $service->id]) }}" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')"
...
@section('javascript')
    <script>
        ...
        $('#type_tinh_cong_no').change(function (e) { 
            e.preventDefault();
            $.ajax({
                        url: '{{route('admin.service.building.set_type_tinh_cong_no')}}',
                        type: 'POST',
                        data: {
             ...
             $('.order_index_accounting').click(function(){
                ...
                function sendOrderToServer() {
                    ...
                    $.ajax({
                        type: "POST", 
                        dataType: "json", 
                        url: "{{ route('admin.service.building.update_index_accounting') }}",
                        data: {
                            order: order,
                            _token: token
                        },
