                                ...
                                <div class="box-header with-border">
                                    <div class="row form-group">
                                        <div class="col-sm-8 col-xs-12 ">
                                            <a href="{{ action('System\ConfigSendInvoiceController@create') }}" type="buttom" class="btn btn-info"><i
...
{{--                                    </form>--}}
                ...
                <span class="btn-group">
                    <a data-action="{{ action('System\ConfigSendInvoiceController@ajaxDeleteMulti') }}" class="btn btn-danger"
                                                ...
                                                @if($config_send_mails->count())
                                                    @foreach($config_send_mails as $key => $config_send_mail)
                                                        <tr>
                                                            ...
                                                            <td>{{config('send_mail.invoice')[$config_send_mail->status]}}</td>
                                                            <td>{{@$config_send_mail->mailTemplate->name}}</td>
                                                            <td>
                                                                <a href="{{ action('System\ConfigSendInvoiceController@edit', ['id' => $config_send_mail->id]) }}" type="button"
                                                                   class="btn btn-sm btn-info" title="Sửa"><i class="fa fa-edit"></i></a>

                                                                <a title="Xóa" href="javascript:;"
                                                                   data-url="{{ action('System\ConfigSendInvoiceController@destroy', ['id' => $config_send_mail->id] ) }}" data-id="{{ $config_send_mail->id }}"
