...
<section class="content">
    <div class="box box-primary">
        <div class="box-body ">
            @if( in_array('admin.feedbackform.index',@$user_access_router))
                <form id="form-search" action="{{ route('admin.feedbackform.index') }}" method="post">
                    {{ csrf_field() }}
                    <div class="row form-group">
                        ...
                        <span class="btn-group">
                            <a href="{{ route('admin.feedbackform.create') }}" class="btn btn-success"><i class="fa fa-plus"></i>&nbsp;&nbsp;Thêm mới</a>
                        </span>
                        </div>
            ...
            @endif
            @if( in_array('admin.feedbackform.index',@$user_access_router))
                    <form id="form-search-advance" action="{{ route('admin.feedbackform.index') }}" method="post">
                        {{ csrf_field() }}
            ...
            @endif
            @if( in_array('admin.feedbackform.action',@$user_access_router))
                    <form id="form-feedback" action="{{ route('admin.feedbackform.action') }}" method="post">
                        @csrf
                            ...
                            <table class="table table-hover table-striped table-bordered">
                                ...
                                @foreach ($lists as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.feedbackform.download')}}?url={{ $item->url }}">Tải xuống</a>
                                        </td>
                                        <td>
                                            @if( in_array('admin.feedbackform.action',@$user_access_router))
                                                <a title="Thay đổi trạng thái" href="javascript:;" data-url="{{ route('admin.feedbackform.action') }}" data-id="{{ $item->id }}" data-status="{{ $item->status }}" class="btn-status label label-sm label-{{ $item->status ? 'success' : 'danger' }}">
                                                    {{ $item->status ? 'Hiện' : 'Ẩn' }}
                                                </a>
                                            @else
                                        ...
                                        <td>
                                            @if( in_array('admin.feedbackform.action',@$user_access_router))
                                                <a title="Sửa" href="{{ route('admin.feedbackform.edit', ['id' => $item->id]) }}" class="btn btn-sm btn-info"><i class="fa fa-edit"></i></a>
                                                <a title="Xóa" href="javascript:;" data-url="{{ route('admin.feedbackform.action') }}" data-id="{{ $item->id }}" class="btn btn-sm btn-delete btn-danger"><i class="fa fa-trash"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
