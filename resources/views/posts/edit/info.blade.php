...
<div class="box box-primary">
    ...
    <div class="box-body">
        ...
        <div class="form-group">
            ...
            <div>
                @php
                $status = ($id == 0) || ($old ? old('status') : $post->status);
                @endphp
                <label class="switch">
        ...
        <div class="form-group">
            @if ($user->can($per_update, app(App\Models\Post::class)))
            <button type="submit" class="btn btn-success" form="form-posts" value="submit">{{ $id ? 'Cập nhật' : 'Thêm mới' }}</button>
            @endif
            &nbsp;
            <a href="{{ route('admin.posts.index', ['type' => $type]) }}" class="btn btn-danger" form="form-posts" value="submit">Quay lại</a>
        </div>
    </div>
</div>