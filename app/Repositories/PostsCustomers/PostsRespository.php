...
namespace App\Repositories\PostsCustomers;
...
//        }
        ...
        $default = [
            'select'   => ['id','title','type','start_at','end_at','response', 'publish_at', 'notify','attaches'],
            'where'    => $where,
            'order_by' => 'id DESC',
            'per_page' => $perpage,
        ];
    ...
    public function searchByApiCustomer($building_id,$info,$request='',$where=[],$perpage = 20)
    {
        ...
        $default = [
            'select'   => ['id','title','type','start_at','end_at','response', 'publish_at', 'image', 'user_id', 'notify','attaches','url_video'],
            'where'    => $where,
            'order_by' => 'id DESC',
            'per_page' => $perpage,
        ];
        ...
        if($info){
            $model->Where(function($query) use ($request,$info){
                ...
                if (!empty($request->place)) {
                    $query->orWhereRaw('JSON_CONTAINS(notify->"$.place_ids", \'["'.$request->place.'"]\')');
                }
                foreach ($info->bdcCustomers as $bc){
                    $query->orWhereRaw('JSON_CONTAINS(notify->"$.group_ids", \'["'.@$bc->bdcApartment->id.'"]\')');
                    $query->orWhereRaw('JSON_CONTAINS(notify->"$.floor_ids", \'["'.@$bc->bdcApartment->floor.'"]\')');
                }
            });
        }
    ...
    public function selectByApi(array $select,$column,$post_id,$building_id,$info='')
    {
        ...
        $info_post = $info_post->with(['user.profileAll' => function($query) use ($building_id){
            $query->where('bdc_building_id', $building_id);
        }])->first();
        if($info_post){
            return $info_post->toArray();
        }
        return [];
    }
