...
class BanksRespository extends Repository {
    ...
    function model()
    {
        return \App\Models\Banks\Banks::class;
    }
...
//        dd($list_search);
    ...
    public function searchByApi($building_id,$request='',$where=[],$perpage = 20,$id = 0)
    {
        ...
        $model= $model->where('bdc_building_id',$building_id);
        $list_search = $model->orderByRaw($options['order_by'])->paginate($options['per_page']);
        $list_search->load('pubUserProfile','allComments','pubUserProfile.bdcCustomers','bdcApartment','pubUserProfile.bdcDepartmentStaff','pubUserProfile.bdcDepartmentStaff.department');
