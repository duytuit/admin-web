...
class FcmController extends BuildingController
{
    ...
    public function seeAllNotiCus(Request $request,$id=0)
    {
        ...
        // $count = $this->modelLog->findByStatusVsSee($info);
        return $this->responseSuccess(['count'=>1]);
    }
    ...
    public function saveTokenCus(Request $request)
    {
        ...
        $user_old = Users::where(function($query) use($user){
            if($user->email){
        ...
        if($user_old){
            $this->modelFcm->deletefcm($user_old->id,$request->type ?? 'cudan');
            $action = $this->modelFcm->newToken($user_old->id,$request->id,$request->token,$request->type ?? 'cudan');
            if($action){
                return $this->responseSuccess(['Cập nhật thành công']);
            }
        }
    ...
    public function pushNotify(Request $request)
    {
        ...
        if($user_token && $data_payload){
            ...
            foreach ($fcms as $key => $value) {
                SendNotifyFCMService::pushNotify($value->token,$data_payload,$user_token->bundle_id);
            }
