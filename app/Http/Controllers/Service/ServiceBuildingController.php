...
class ServiceBuildingController extends BuildingController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('service.building.index', $data);
    }
    ...
    public function set_type_tinh_cong_no(Request $request)
    {
        ...
        $dataResponse = [
            ...
            'href' => route('admin.service.building.index')
        ];
    ...
    public function create()
    {
        ...
        return view('service.building.create', compact(['meta_title', 'priceTypes', 'progressives','typeService']));
    }
    ...
    public function edit($id)
    {
        ...
        $typeService = Category::where(['type'=>'service','bdc_building_id'=>$this->building_active_id,'status'=>1])->get();
        return view('service.building.edit', compact(['service', 'meta_title', 'priceTypes', 'progressives','typeService']));
    }
    ...
    public function update(ServiceRequest $request, $id)
    {
        ...
        return redirect()->route('admin.service.building.index')->with('success', 'Sửa dịch vụ thành công.');
    }
    public function destroy($id)
    {
        try {
            ...
            DB::rollBack();
            return redirect()->route('admin.service.building.index')->with('error', 'Xóa thất bại.');
        }
        return redirect()->route('admin.service.building.index')->with('success', 'Xóa dịch vụ thành công.');
    }
    ...
    public function choose()
    {
        ...
        $data['per_page'] = Cookie::get('per_page', 10);
        return view('service.building.choose', compact('meta_title'), $data);
    }

    public function changeStatus(Request $request)
    {
        $service = $this->serviceRepo->getServiceById($request->id);
        // if ($service->type == 2) {
        //     $dataResponse = [
        //         'error' => true,
        //         'message' => 'Không thể thay đổi trang thái dịch vụ này!'
        //     ];
        //     return response()->json($dataResponse);
        // }
    ...
    public function postChoose(Request $request)
    {
        ...
        return redirect()->route('admin.service.building.index')->with('success', 'Thêm dịch vụ mới thành công.');
    }
    ...
    public function importExcel()
    {
        ...
        return view('service.building.import_excel', $data);
    }
    ...
    public function importApartmentService(
        ...
        if(!$file) return redirect()->route('admin.service.building.importexcel')->with('warning', 'Chưa có file upload!');
        ...
        if ($excel_data->count()) {
            ...
            foreach ($excel_data as $content) {
                ...
                try {
                    $serviceApart = $this->_apartmentServicePriceRepository->create($data);
                    ApartmentServicePrice::where('id','<>', $serviceApart->id)->where(['bdc_price_type_id'=>2,'bdc_apartment_id'=>$serviceApart->bdc_apartment_id])->whereHas('service',function($query) use($serviceApart){
                        $query->where('type',@$serviceApart->service->type);
                    })->update(['status'=>0]);
                    $new_content = $content->toArray();
