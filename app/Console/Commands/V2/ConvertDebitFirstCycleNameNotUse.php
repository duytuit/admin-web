...
class ConvertDebitFirstCycleNameNotUse extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'convertdebitfirstcyclename_cron_not_use';
    ...
    public function handle()
    {
        try {
            $discount_total=0;
            do {
                $value = QueueRedis::getItemForQueue("add_queue_insert_debit_v4_2");
                $value = (object)$value;
                if(!$value){
                    break;
                }
                        ...
                        if($discount > 0){
                            ...
                            tb.cycle_name,tb.bdc_bill_id,tb.bdc_apartment_service_price_id
                             ...
                             GROUP BY bdc_bill_id , bdc_apartment_service_price_id , bdc_apartment_id) AS tb1 ON 
                                 ...
                                 AND tb.deleted_at IS NULL and (tb.version >= 0 or tb.new_sumery >= $discount) and tb.sumery > 0";
