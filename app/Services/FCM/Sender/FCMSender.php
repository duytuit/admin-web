...
use App\Services\FCM\DXCustomRequest;
...
class FCMSender extends HTTPSender
{
     ...
     */
    public function sendTo($to, Options $options = null, PayloadNotification $notification = null, PayloadData $data = null, $config = 'fcm.http')
    {
        $response = null;
        ...
        if (is_array($to) && !empty($to)) {
            ...
            foreach ($partialTokens as $tokens) {
                $request = new DXCustomRequest($tokens, $options, $notification, $data);
                $request = $request->setConfigName($config);
            ...
            $request = new DXCustomRequest($to, $options, $notification, $data);
            $request = $request->setConfigName($config);
            $responseGuzzle = $this->post($request);
     ...
     */
    public function sendToGroup($notificationKey, Options $options = null, PayloadNotification $notification = null, PayloadData $data = null, $config = 'fcm.http')
    {
        $request = new DXCustomRequest($notificationKey, $options, $notification, $data);
     ...
     */
    public function sendToTopic(Topics $topics, Options $options = null, PayloadNotification $notification = null, PayloadData $data = null, $config = 'fcm.http')
    {
        $request = new DXCustomRequest(null, $options, $notification, $data, $topics);
        $request = $request->setConfigName($config);
        $responseGuzzle = $this->post($request);
